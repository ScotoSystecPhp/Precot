$(document).ready(function () {
    ...
    $("#Date").val(currentDate);
		...
		success: function (response) {
			...
			$("#Department").empty();
			$("#Department").append($("<option></option>").attr("value", "").text("---Select---"));
            $("#sub_Department").append($("<option></option>").attr("value", "").text(""));
            $("#JobCardNo").empty().append($("<option></option>").attr("value", "").text(""));

            $("#Shift_Employee").append($("<option></option>").attr("value", "").text(""))
                .append($("<option></option>").attr("value", "GENERAL").text("GENERAL"))
                .append($("<option></option>").attr("value", "SHIFT").text("SHIFT"));

            $("#Shift").append($("<option></option>").attr("value", "").text(""))
            .append($("<option></option>").attr("value", "GENERAL").text("GENERAL"))
            .append($("<option></option>").attr("value", "SHIFT1").text("SHIFT1"))
            .append($("<option></option>").attr("value", "SHIFT2").text("SHIFT2"))
            .append($("<option></option>").attr("value", "SHIFT3").text("SHIFT3"));
    ...
    $.ajax({
		...
		if ($("#JobCardNo option").length > 1) {
        $("#PreviousdaydBtn").show();
        $("#NotPreviousdaydBtn").show();
    } else {
        $("#PreviousdaydBtn").hide();
        $("#NotPreviousdaydBtn").hide();
    }
    ...
    $("#Shift").on("change", function () {
        ...
        var Department = $("#Department").val();
        var Shift_Type = $("#Shift_Employee").val();
        var Date = $("#Date").val();
        var Shift = $("#Shift").val();

        $.ajax({
			url: baseurl + "Allocation/Shift_Employee_Late",
			type: "POST",
			data: {
				Department: Department,
				Shift: Shift,
			...
			success: function (response) {
				var responseData = JSON.parse(response);
				var Employee_Data = responseData.Shift_Employee_Late;
...
function handleFrameTypeChange(frameType) {
	...
	$("#Work_Allocation").on("click", function () {
        ...
        var ShiftType = $("#Shift_Employee").val();
			...
			success: function (response) {
				...
				if (
					...
					$("#Work_Allocation_Clear").on("click", function () {
						...
						fetchEmployeeData(Department, Shift, Date,ShiftType);
					...
					if (
						assignedMachine &&
						(Machine_Id === "NoWork" || Machine_Id === "Others")
					) {
						swal({
							title: "Error!",
							text: "This employee already has a machine assigned. They cannot be assigned 'NoWork' or 'Others'.",
							icon: "error",
							buttons: true,
							className: "swal-small",
						});
					...
					var machineAndFrameDuplicate = false;
					$("#Allocation_Table tbody tr").each(function () {
						var row = $(this);
						var rowMachineId = row.find("td:eq(1)").text();
						var rowWorkType = row.find("td:eq(4)").text();

						if (rowMachineId === Machine_Id && rowWorkType === WorkType) {
							if (
								Machine_Id !== "NoWork" &&
								Machine_Id !== "Others" &&
								...
								machineAndFrameDuplicate = true;
								return false;
							}
						}
					});

					if (machineAndFrameDuplicate) {
						swal({
							title: "Error!",
							text: "This machine has already been allocated to this employee with the same work type and frame.",
							icon: "error",
							buttons: true,
							className: "swal-small",
						});
					...
					if (duplicateFound) {
						swal({
							title: "Error!",
							text: "This employee has already been assigned to 'NoWork' or 'Others'. Please check the details.",
							icon: "error",
							buttons: true,
							className: "swal-small",
						});
        ...
        function fetchEmployeeData(Department, Shift, Date,ShiftType) {
...
$("#Work_Allocations").on("click", function () {
    ...
    swal({
        ...
        if (willDelete) {
            ...
            $.ajax({
                url: baseurl + "Allocation/delete",
                type: "POST",
                contentType: "application/json",
                data: JSON.stringify(formData),
                success: function (response) {
                    if (!response || response.trim() === "") {
                        console.error("Received empty response from delete request.");
                        return;
                    }
                    ...
                    if (responseData.status === "success") {
                        swal({
                            ...
                            // Remove the row and reload data
                            row.remove();
                            reloadFormDatas();
                            ...
                            $.ajax({
                                ...
                                success: function (response) {
                                    ...
                                    // Destroy existing DataTable instance and reinitialize after appending rows
        ...
        $.ajax({
            ...
            success: function (response) {
                try {
                    ...
                    $.each(Employee_Name, function (key, value) {
                        $("#Employee_Id").append($("<option></option>").attr("value", key).text(key + " => " + value));
                    });
                } catch (error) {
